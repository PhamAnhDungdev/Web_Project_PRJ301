<%-- 
    Document   : cart
    Created on : Feb 24, 2024, 5:29:54 PM
    Author     : Bravo 15
--%>
...
<html lang="en">
    ...
    <%
        ...
        HttpSession sess = request.getSession();//get user
        ...
        List<Products> listp = null; //list sản phẩm để hiển thị
        List<CartItem> listcart = null; //list giỏ hàng để lấy số lượng của từng sản phẩm
        
        Cart o = cart.getCartByUid(x.getId());//get giỏ hàng theo userID từ cart
        if(x != null){
            if(u.getListByUserId(x.getId()) == null){
                listp = null;
            }else{
                        ...
                        <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                            <div class="navbar-nav mr-auto py-0">
                                ...
                                <div class="nav-item dropdown">
                                    <a href="home.jsp" class="nav-link dropdown-toggle" data-toggle="dropdown">Lựa chọn<i class="fa fa-angle-down mt-1"></i></a>
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <a href="cart.jsp" class="btn px-0 ml-3">
                                            <i class="fas fa-shopping-cart text-primary"></i>
                                            <span class="badge text-secondary border border-secondary rounded-circle" style="padding-bottom: 2px;"><%=listp.size()%></span>
                                        </a>
                                    </c:otherwise>
                                </c:choose>
                            </div>
            ...
            <div class="row px-xl-5">
                <div class="col-12">
                    <nav class="breadcrumb bg-light mb-30">
                        ...
                        <a class="breadcrumb-item text-dark" href="cart.jsp">Giỏ hàng</a>
                ...
                <div class="col-lg-8 table-responsive mb-5">
                    <table class="table table-light table-borderless table-hover text-center mb-0">
                        ...
                        <%if(sumprice != 0) {%>
                            ...
                            <tr>
                                <td class="align-middle"><img src="image/<%=listp.get(i).getImage()%>" alt="" style="width: 50px;"> <%=listp.get(i).getTitle()%></td>
                                <td class="align-middle"><%=listp.get(i).getPrice()%></td>
                                <td class="align-middle"><%=listcart.get(i).getQuantity()%></td>
                                <td class="align-middle"><%=listp.get(i).getPrice()*listcart.get(i).getQuantity()%></td>
                                <form action="deletecartitem" method="GET">
                                    <td class="align-middle"><button type="submit" name="pid" value="<%=listp.get(i).getId()%>" class="btn btn-sm btn-danger"><i class="fa fa-times"></i></button></td>
                                </form>
                            </tr>
                            <%}%>
                        <%}else{%>
                        <h4>Giỏ hàng trống. Tiếp tục mua sắm tại <a style="text-decoration: none;" href="shop.jsp">cửa hàng</a></h4> </br>
                        <%}%>
                        </tbody>
                    </table>
                </div>
                <div class="col-lg-4">
                    ...
                    <div class="bg-light p-30 mb-5">
                        ...
                        <div class="pt-2">
                            ...
                            <%}%>
                            <!--<button type="submit" class="btn btn-block btn-primary font-weight-bold my-3 py-3">Đi tới đặt hàng</button>-->
                            <%if(listcart.size() == 0){%>
                            <button type="submit" class="btn btn-block btn-primary font-weight-bold my-3 py-3">Mua hàng</button>
                            <%} else {%>
